...
Besonders kritisch ist dieser „Information Drop“ in der hier betrachteten Angebotsphase, da Bauunternehmen zu diesem Zeitpunkt kaum Einfluss auf die Modellerstellung durch Dritte (Planer) nehmen können \cite{borrmann2015building}. Modelle von Planern sind zwar oft geometrisch detailliert, verfügen jedoch meist nicht über die baubetriebliche Informationstiefe, die für eine automatisierte Mengenermittlung notwendig wäre. Da die Modellvorgaben unternehmensübergreifend oft nicht vollständig steuerbar sind, muss das Bauunternehmen mit einem Modell arbeiten, dessen Informationsgehalt für interne Kalkulationsprozesse unzureichend ist.
...
Für den in dieser Arbeit entwickelten Algorithmus steht vor allem die \textbf{alphanumerische Informationstiefe} im Fokus. Da nachfolgende Gewerke spezifische semantische Kriterien benötigen, um auf Modellen anderer Disziplinen aufzubauen, ist ein gezieltes „Nachrüsten“ (Retrofitting) von Parametern notwendig \cite{abualdenien2022levels}. Der Algorithmus adressiert somit den Bedarf an strukturierten, nicht-geometrischen Daten, die für die Kalkulation essenziell sind.
...
Die Vererbungshierarchie ist für den Anreicherungs-Algorithmus von zentraler Bedeutung, da sie es ermöglicht, Informationsanforderungen generisch auf höheren Ebenen (z. B. für alle \texttt{IfcBuildingElement}) zu definieren, anstatt diese für jede spezifische Subklasse redundant festzulegen. Dies erhöht die Robustheit und Skalierbarkeit der automatisierten Datenprüfung und -ergänzung.
...
Während Property Sets qualitative Informationen liefern, dienen \texttt{IfcQuantitySets} der Mengenermittlung. In der Praxis zeigt sich hier jedoch oft eine Diskrepanz zu nationalen Kalkulationsstandards wie dem österreichischen Leistungsbuch Hochbau (LBH) oder der ÖNORM B 2114. Diese Diskrepanz entsteht vor allem dadurch, dass traditionelle Rechenregeln der Bauabrechnung (z. B. nach ÖNORM B 2114 oder LBH) auf manuellen Arbeitsweisen basieren, bei denen Öffnungen bis zu einer bestimmten Größe übermessen werden. Während BIM-Modelle Bauteile meist geometrisch korrekt („Netto“) abbilden, erfordert die Kalkulation oft „Abrechnungsmaße“, die über die reine geometrische Repräsentation hinausgehen \cite{borrmann2015building}. Eine Erweiterung der Datenstrukturen ist daher unumgänglich, um diese baubetrieblichen Logiken im digitalen Modell abzubilden.
...
Zusammenfassend bilden die erläuterten Grundlagen des OpenBIM-Ökosystems, insbesondere die Struktur des \gls{ifc}-Schemas und die Logik der \gls{ids}, den notwendigen Rahmen für die technische Lösung des beschriebenen Informationsverlusts. Während dieses Kapitel die theoretischen und softwareseitigen Werkzeuge definiert hat, beschreibt das folgende Kapitel~\ref{sec:methodology} die wissenschaftliche Methodik, mit der auf dieser Basis ein systematischer Anreicherungs-Algorithmus entwickelt und evaluiert wird.
